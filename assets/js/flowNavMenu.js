/** @param {"board" | "completed" | "charts" | "admin"} kind */
...
/**
 * Header hamburger: Board, Completed, Charts, Admin.
 * @param {{ current: "board" | "completed" | "charts" | "admin" }} opts
 * @returns {HTMLElement}
 */
export function createFlowNavMenu(opts) {
  const { current } = opts;
  /** @type {{ board: string, completed: string, charts: string, admin: string }} */
  let hrefs;
  switch (current) {
    case "board":
      hrefs = {

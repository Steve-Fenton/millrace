/** @param {"board" | "completed" | "charts" | "preferences" | "admin"} kind */
...
/**
 * Header hamburger: Board, Completed, Charts, Preferences, Admin.
 * @param {{ current: "board" | "completed" | "charts" | "preferences" | "admin" }} opts
 * @returns {HTMLElement}
 */
export function createFlowNavMenu(opts) {
  const { current } = opts;
  /** @type {{ board: string, completed: string, charts: string, preferences: string, admin: string }} */
  let hrefs;
  switch (current) {
    case "board":
      hrefs = {
